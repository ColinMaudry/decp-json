...
jq -c -M --arg datetime "$datetime" --arg datasetUrl "$dataset_url" --arg ocidPrefix "$ocid_prefix" --arg packageUri "$package_uri" -f scripts/jq/ocds/decp2ocds.jq "$1" | sed -e 's/"[^"]*":\(null\|\[\]\|{}\),\?//g' -e 's/,}/}/g'
